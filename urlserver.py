...
def urlserver_get_base_url():
    ...
    if urlserver_settings['http_port_display']:
    ...
    if urlserver_settings['http_url_prefix']:
        prefix = '{}/'.format(urlserver_settings['http_url_prefix'])

    return '{}://{}{}/{}'.format(scheme, hostname, prefixed_port, prefix)


def urlserver_short_url(number, full=True):
    """Return short URL with number."""
    return '{}{}'.format(urlserver_get_base_url() if full else '',
...
def urlserver_server_reply(conn, code, extra_header, message,
    ...
    s = ('HTTP/1.1 {}\r\n'
         ...
         'Content-Type: {}\r\n'
         'Content-Length: {}\r\n'
         '\r\n'.format(code, extra_header, mimetype, len(message)))
    msg = None
    # python 3.x
    ...
    if urlserver_settings['debug'] == 'on':
        weechat.prnt('', 'urlserver: sending {} bytes'.format(len(msg)))
    conn.sendall(msg)
...
def urlserver_server_reply_list(conn, sort='-time'):
    ...
    for column, defaultsort in (('time', '-'), ('nick', ''), ('buffer', '')):
        if sort[1:] == column:
            content += ('<th class="sortable sorted_by {0}_header">'
                        '<a href="sort={1}{0}">{2}</a> {3}</th>'.format(
                            column,
                            sortkey[sort[0]][0],
                            column.capitalize(),
                            sortkey[sort[0]][1]))
        else:
            content += ('<th class="sortable {0}_header">'
                        '<a class="sort_link" href="sort={1}{0}">{2}</a></th>'
                        .format(
                            column,
                            defaultsort,
                            column.capitalize()))
    ...
    for key, item in urls:
        ...
        message = (html.escape(item[4].replace(url, '\x01\x02\x03\x04'))
                   .split('\t', 1))
        message[0] = '<span class="prefix">{}</span>'.format(message[0])
        message[1] = '<span class="message">{}</span>'.format(message[1])

        strjoin = '<span class="prefix_suffix"> {} </span>'.format(
        ...
        message = strjoin.join(message).replace(
            '\x01\x02\x03\x04',
            '</span><a class="url" href="{0}" title="{1}"{2}>{1}'
            '</a><span class="message">'.format(
                urlserver_short_url(key, False), url, target))
        if urlserver_settings['http_embed_image'] == 'on' and \
                url.lower().endswith(('.jpg', '.jpeg', '.png', '.gif',
                                      '.bmp', '.svg')):
            obj = ('<div class="obj"><img src="{0}" title="{0}" alt="{0}">'
                   '</div>'.format(url))
            ...
            if m:
                ...
                obj = ('<div class="obj youtube">'
                       '<iframe id="{}" type="text/html" width="%d" '
                       'height="%d" '
                       'src="https://www.youtube.com/embed/{}?enablejsapi=1">'
                       '</iframe></div>' % (yid, width, height, yid))
        content += ('<td class="timestamp">{}</td>'
                    '<td class="nick">{}</td>'
                    '<td class="buffer">{}</td><td class="message">'.format(
                        item[0], item[1], item[2]))
        content += '{}{}</td></tr>\n'.format(message, obj)
    content += '</table>'
    if urlserver_settings['http_css_url']:
        css = '<link rel="stylesheet" type="text/css" href="{}" />'.format(
    ...
    html_message = ('<html>\n'
                    ...
                    '</html>'.format(
                        ...
                        content))
    urlserver_server_reply(conn, '200 OK', '', html_message)
...
def urlserver_server_fd_cb(data, fd):
    ...
    if urlserver_settings['debug'] == 'on':
        weechat.prnt('', 'urlserver: connection from {}'.format(str(addr)))
    ...
    if m:
        url = m.group(1)
        if urlserver_settings['debug'] == 'on':
            weechat.prnt('', 'urlserver: {}'.format(m.group(0)))
        if 'favicon.' in url:
...
def urlserver_server_status():
    ...
    global urlserver
    if urlserver['socket']:
        weechat.prnt('', 'URL server listening on {}'.format(
...
def urlserver_server_start():
    ...
    try:
        ...
        weechat.prnt('', '{}Bind error: {}'.format(weechat.prefix('error'), e))
    ...
    if urlserver_settings['display_urls'] == 'on':
        ...
        if urls_short:
            if urlserver_settings['separators'] and \
                ...
                separator = ' {} '.format(urlserver_settings['separators'][1])
            ...
            weechat.prnt_date_tags(
                ...
                '{}{}'.format(weechat.color(urlserver_settings['color']),
                              urls_string))
...
def urlserver_modifier_irc_cb(data, modifier, modifier_data, string):
    ...
    info_name = '{},{}'.format(modifier_data, msg['channel'])
