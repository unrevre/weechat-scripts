...
#     version 0.8: add option autoset.look.timer to add a small timer
...
def as_config_init():
    ...
    Sections: buffer.
    """
    global as_config_file, as_options
    as_config_file = weechat.config_new(CONFIG_FILE_NAME,
                                        "as_config_reload_cb", "")
    if as_config_file == "":
        return
    ...
    section_look = weechat.config_new_section(
        as_config_file, "look", 0, 0, "", "", "", "", "", "", "", "", "", "")
    if not section_look:
        weechat.config_free(as_config_file)
        return

    # options in section "look"
    as_options["look_timer"] = weechat.config_new_option(
        as_config_file, section_look, "timer", "integer",
        "Timer used to delay the set of properties (in milliseconds, "
        "0 = don't use a timer)",
        "", 0, 2147483647, "1", "1", 0, "", "", "", "", "", "")

    as_options["look_instant"] = weechat.config_new_option(
        as_config_file, section_look, "instant", "boolean",
        "Instantly apply properties to buffers affected",
        "", 0, 0, "on", "on", 0, "", "", "", "", "", "")

    # section "buffer"
    section_buffer = weechat.config_new_section(
        as_config_file, "buffer", 1, 1, "", "", "", "", "", "",
        "as_config_buffer_create_option_cb", "", "", "")
    if not section_buffer:
        weechat.config_free(as_config_file)
        return


def as_config_buffer_create_option_cb(data, config_file, section, option_name,
                                      value):
...
def as_config_reload_cb(data, config_file):
...
def as_config_read():
    ...
    return weechat.config_read(as_config_file)


def as_config_write():
    """Write configuration file."""
    global as_config_file
    return weechat.config_write(as_config_file)


# ================================[ command ]=================================

def as_cmd(data, buffer, args):
    """Callback for /autoset command."""
    args = args.strip()
    if args == "":
        weechat.command("", "/set {}.buffer.*".format(CONFIG_FILE_NAME))
    ...
    if argv:
        if argv[0] == "add":
            if len(argv) < 4:
                ...
                return weechat.WEECHAT_RC_OK
            weechat.command("", "/set {}.buffer.{}.{} \"{}\"".format(
                CONFIG_FILE_NAME, argv[1], argv[2], argv[3]))
        elif argv[0] == "del":
            if len(argv) < 2:
                weechat.command("", "/help {}".format(SCRIPT_COMMAND))
                return weechat.WEECHAT_RC_OK
            weechat.command("", "/unset {}.buffer.{}".format(
...
def as_completion_current_buffer_cb(data, completion_item, buffer,
                                    completion):
    """
    Complete with current buffer name (plugin.name),
    for command '/autoset'.
    """
    name = "{}.{}".format(weechat.buffer_get_string(buffer, "plugin"),
...
def as_completion_options_cb(data, completion_item, buffer, completion):
    """Complete with config options, for command '/autoset'."""
    options = weechat.infolist_get("option", "",
                                   "{}.buffer.*".format(CONFIG_FILE_NAME))
    if options:
...
def as_apply_options_for_buffer(buffer):
    ...
    while weechat.infolist_next(options):
        ...
        if option:
            ...
            if pos > 0:
                ...
                prop = option[pos+1:]
                if buffer_mask and prop:
                    if weechat.string_match(full_name, buffer_mask, 1):
                        weechat.buffer_set(buffer, prop, value)

    weechat.infolist_free(options)


def as_timer_buffer_opened_cb(data, remaining_calls):
    full_name = data
    buffer = weechat.buffer_search("==", full_name)
    if not buffer:
        return weechat.WEECHAT_RC_OK
    as_apply_options_for_buffer(buffer)
    return weechat.WEECHAT_RC_OK


def as_signal_buffer_opened_cb(data, signal, signal_data):
    global as_options
    buffer = signal_data
    timer = weechat.config_integer(as_options["look_timer"])
    if timer == 0:
        as_apply_options_for_buffer(buffer)
    else:
        weechat.hook_timer(timer, 0, 1,
                           "as_timer_buffer_opened_cb",
                           weechat.buffer_get_string(buffer, "full_name"))
    return weechat.WEECHAT_RC_OK


def as_config_option_cb(data, option, value):
    if not weechat.config_boolean(as_options["look_instant"]):
        return weechat.WEECHAT_RC_OK

    if not weechat.config_get(option):  # option was deleted
        return weechat.WEECHAT_RC_OK

    option = option[len("{}.buffer.".format(CONFIG_FILE_NAME)):]
...
if __name__ == "__main__":
    if weechat.register(SCRIPT_NAME, SCRIPT_AUTHOR, SCRIPT_VERSION,
                        SCRIPT_LICENSE, SCRIPT_DESC, "as_unload_script", ""):
        version = weechat.info_get("version_number", "") or 0
        if int(version) < 0x01000000:
            weechat.prnt("", "{}{}: WeeChat 1.0 is required for this script."
