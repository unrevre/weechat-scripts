...
#   * Add ${info:autosort_replace,from,to,text} to replace substrings in sort
#   * rules.
#   * Add ${info:autosort_order,value,first,second,third} to ease
#   * writing sort rules.
...
#   * Add hint to set irc.look.server_buffer to independent and
#   * buffers.look.indenting to on.
# 2.4:
...
def ensure_str(string):
    '''
    Make sure the given type is the correct string type.
    '''
...
def parse_int(arg, arg_name='argument'):
    ...
    try:
        ...
        raise HumanReadableError('Invalid {0}: expected integer, got "{1}".'
                                 .format(arg_name, arg))

def decode_rules(blob):
    parsed = json.loads(blob)
    if not isinstance(parsed, list):
        log('Malformed rules, expected a JSON encoded list of strings, but '
            'got a {0}. No rules have been loaded. Please fix the setting'
            'manually.'.format(type(parsed)))
        return []

    for i, entry in enumerate(parsed):
        if not isinstance(entry, str):
            log('Rule #{0} is not a string but a {1}. No rules have been '
                'loaded. Please fix the setting manually.'
                .format(i, type(entry)))
            return []
...
def decode_helpers(blob):
    ...
    if not isinstance(parsed, dict):
        log('Malformed helpers, expected a JSON encoded dictonary but got a '
            '{0}. No helpers have been loaded. Please fix the setting '
            'manually.'.format(type(parsed)))
        return {}

    for key, value in parsed.items():
        if not isinstance(value, str):
            log('Helper "{0}" is not a string but a {1}. No helpers have been '
                'loaded. Please fix seting manually.'.format(key, type(value)))
            return {}
    return parsed
...
class Config:
    ...
    def __init__(self, filename):
        ...
        if not self.config_file:
            log('Failed to initialize configuration file "{0}".'
        ...
        self.sorting_section = weechat.config_new_section(
            ...
            '', '', '', '', '', '', '', '', '', '')
        self.v3_section = weechat.config_new_section(
            self.config_file, 'v3', False, False,
            '', '', '', '', '', '', '', '', '', '')
        ...
        self.__rules = weechat.config_new_option(
            ...
            'rules', 'string',
            'An ordered list of sorting rules encoded as JSON. See /help '
            'autosort for commands to manipulate these rules.',
        ...
        self.__helpers = weechat.config_new_option(
            self.config_file, self.v3_section,
            'helpers', 'string',
            'A dictionary helper variables to use in the sorting rules, '
            'encoded as JSON. See /help autosort for commands to manipulate '
            'these helpers.',
        ...
        self.__signals = weechat.config_new_option(
            self.config_file, self.sorting_section,
            'signals', 'string',
            'A space separated list of signals that will cause autosort to '
            'resort your buffer list.',
        ...
        self.__signal_delay = weechat.config_new_option(
            ...
            'Delay in milliseconds to wait after a signal before sorting the '
            'buffer list. This prevents triggering many times if multiple '
            'signals arrive in a short time. It can also be needed to wait '
            'for buffer localvars to be available.',
            '', 0, 1000, str(Config.default_signal_delay),
            str(Config.default_signal_delay), 0,
            '', '', '', '', '', ''
        )
        ...
        self.__sort_on_config = weechat.config_new_option(
            ...
            'Decides if the buffer list should be sorted when autosort '
    ...
    def save_helpers(self, run_callback=True):
        ...
        weechat.config_option_set(self.__helpers, json.dumps(self.helpers),
                                  run_callback)
...
def buffer_sort_key(rules, helpers, case_sensitive):
    ...
    def key(buffer):
        ...
        for helper_name, helper in sorted(helpers.items()):
            expanded = weechat.string_eval_expression(
                helper, {"buffer": buffer}, {}, {})
            if not case_sensitive:
                expanded = expanded.casefold()
            extra_vars[helper_name] = expanded
        result = []
        for rule in rules:
            expanded = weechat.string_eval_expression(
                rule, {"buffer": buffer}, extra_vars, {})
            if not case_sensitive:
                expanded = expanded.casefold()
...
def split_args(args, expected, optional=0):
    ...
    if len(split) < expected:
        raise HumanReadableError('Expected at least {0} arguments, got {1}.'
                                 .format(expected, len(split)))
    return split[:-1] + pad(split[-1].split(' ', optional), optional + 1, '')

def do_sort():
    hdata, buffers = get_buffers()
    buffers = merge_buffer_list(buffers)
    buffers = sort_buffers(hdata, buffers, config.rules, config.helpers,
...
def command_debug(buffer, command, args):
    ...
    for merged in buffers:
        for buf in merged:
            fullname = weechat.hdata_string(hdata, buf, 'full_name')
            results.append((fullname, key(buf)))
    elapsed = time.perf_counter() - start
...
def command_helper_list(buffer, command, args):
    ...
    width = max(map(lambda x: len(x) if len(x) <= 30 else 0,
                    config.helpers.keys()))

    for name, expression in sorted(config.helpers.items()):
        output += '    {0:>{width}}: {1}\n'.format(
            name, expression, width=width)
...
def parse_args(args, maximum=None):
    result = []
    i = 0
    while maximum is None or i < maximum:
        i += 1
...
def add_completions(completion, words):
    for word in words:
        weechat.hook_completion_list_add(completion, word, 0,
...
def autosort_complete_rules(words, completion):
    ...
    if len(words) == 1 and words[0] in ('delete', 'insert', 'move', 'swap',
                                        'update'):
...
{*white}# Description
Autosort is a weechat script to automatically keep your buffers sorted. The
sort order can be customized by defining your own sort rules, but the default
should be sane enough for most people. It can also group IRC channel/private
buffers under their server buffer if you like.

{*white}# Sort rules{reset}
Autosort evaluates a list of eval expressions (see {*default}/help eval{reset})
and sorts the buffers based on evaluated result. Earlier rules will be
considered first. Only if earlier rules produced identical results is the
result of the next rule considered for sorting purposes.

You can debug your sort rules with the `{*default}/autosort debug{reset}`
command, which will print the evaluation results of each rule for each buffer.

{*brown}NOTE:{reset} The sort rules for version 3 are not compatible with
version 2 or vice versa. You will have to manually port your old rules to
version 3 if you have any.

{*white}# Helper variables{reset}
You may define helper variables for the main sort rules to keep your rules
readable. They can be used in the main sort rules as variables. For example, a
helper variable named `{cyan}foo{reset}` can be accessed in a main rule with
the string `{cyan}${{foo}}{reset}`.
...
You can escape commas and backslashes inside the arguments by prefixing them
with a backslash.
...
cause your buffer list to be sorted. Simply edit the
  `{cyan}autosort.sorting.signals{reset}`
option to add or remove any signal you like.

If you remove all signals you can still sort your buffers manually with the
`{*default}/autosort sort{reset}` command. To prevent all automatic sorting,
the option `{cyan}autosort.sorting.sort_on_config_change{reset}` should also be
disabled.
...
If you are using the {*default}buflist{reset} plugin you can (ab)use Unicode to
draw a tree structure with the following setting (modify to suit your need):
...
if weechat.register(SCRIPT_NAME, SCRIPT_AUTHOR, SCRIPT_VERSION, SCRIPT_LICENSE,
    ...
    weechat.hook_command('autosort', command_description.format(**colors),
                         '', '',
                         '%(plugin_autosort) %(plugin_autosort) '
                         '%(plugin_autosort) %(plugin_autosort) '
                         '%(plugin_autosort)', 'on_autosort_command', '')
    weechat.hook_info('autosort_replace',
                      'Replace all occurences of `from` with `to` in the '
                      'string `text`.',
                      'from,to,text', 'on_info_replace', '')
    weechat.hook_info('autosort_order',
                      'Get a zero padded index of a value in a list of '
                      'possible values. If the value is not found, the index '
                      'for `*` is returned. If there is no `*` in the list, '
                      'the highest index + 1 is returned.',
                      'value,first,second,third,...', 'on_info_order', '')
